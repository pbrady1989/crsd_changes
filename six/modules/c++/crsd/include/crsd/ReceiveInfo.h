...
struct ReceiveInfo
{
    ...
    //! Equality operators
    bool operator==(const ReceiveInfo& other) const
    {
    ...
    //! (Required) Identifies the Receive sensor that collected the
    //! initial PHD or receive only data used to form the
    //! receive signal arrays
    std::string sensorName;

    //! (Required) Identifies the Receive sensor imaging event or
...
//! Ostream operator
std::ostream& operator<< (std::ostream& os, const ReceiveInfo& p);
}
