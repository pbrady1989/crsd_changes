...
#ifndef __CRSD_CRSD_WRITER_H__
#define __CRSD_CRSD_WRITER_H__
#pragma once
...
/*
 *  \class CRSDWriter
 *  \brief CRSD write handler
 *
 *  Used to write a CRSD file. You must be able to provide the
...
struct CRSDWriter
{
    /*
     *  \func Constructor
     *  \brief Sets up the internal structure of the CRSDWriter
     *
     ...
     *         written. The data.arraySize and data.numCRSDChannels will be
    ...
    /*
     *  \func write
     *  \brief Writes the complete CRSD into the file.
     *
     *  This only works with valid CRSDWriter data types:
     ...
     */
    template <typename T>
    void writeCRSDData(const T* data,
                       size_t numElements,
                       size_t channel = 1);
...
private:
     ...
     */
    void writeCRSDDataImpl(const std::byte* data,
                           size_t size);

    /*
     *  Implementation of write compressed wideband
     */
    void writeCompressedCRSDDataImpl(const std::byte* data,
                                     size_t channel);
    ...
    //! Output stream contains CRSD file
    std::shared_ptr<io::SeekableOutputStream> mStream;
};
}
